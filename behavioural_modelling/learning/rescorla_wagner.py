...
def asymmetric_rescorla_wagner_update(
    ...
    outcome_chosen: Tuple[
        Union[jax.typing.ArrayLike, float], jax.typing.ArrayLike
    ],
    alpha_p: float,
    alpha_n: float,
    choice_stickiness: Optional[float] = 0,
    ...
    Value estimates are only updated for chosen actions, assuming choices
    are coded as 0 = unchosen, 1 = chosen. For unchosen actions, the
    prediction error is mulitplied by 0, meaning that it is not updated.
    This behaviour can be modified by coding choices as -1 = unchosen,
    1 = chosen. This allows for the unchosen option to be updated with
    a prediction error of the opposite sign.

    Choice stickiness is implemented by inflating the value of the chosen
    action for the subsequent trial. The corresponding parameter determines
    how much the value of the chosen action is inflated.

    Args:
        value (jax.typing.ArrayLike): The current estimated value of a
            state or action.
        outcome_chosen (Tuple[Union[jax.typing.ArrayLike, float],
            jax.typing.ArrayLike]): A tuple containing the outcome and the
            chosen action. The outcome can be a float or an array (e.g.,
            for a single outcome or multiple outcomes). The chosen action
            is a one-hot encoded array of shape (n_actions,) where 1
            indicates the chosen action and 0 indicates the unchosen
            actions (although see above for alternative coding).
        alpha_p (float): The learning rate used when the
            prediction error is positive.
        alpha_n (float): The learning rate used when the
            prediction error is negative.
        choice_stickiness (Optional[float], optional): The stickiness
            parameter that determines how much the value of the
            chosen action is inflated. Defaults to 0.
    ...
    # Inflate the value of the chosen action for the subsequent trial
    updated_value = updated_value + (choice_stickiness * chosen)
